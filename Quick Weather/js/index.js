...
var moonburst = Snap.select('#moonburst');
...
var weather = [
	...
	{ type: 'sun', name: 'Sunny'},
	{ type: 'cloud', name: 'Clouds' },
	{ type: 'night', name: 'Night' }
];
...
(function() {
	...
	fullDate += weekday[date.getDay()] + " ";

	const monthNames = ["January", "February", "March", "April", "May", "June",
  "July", "August", "September", "October", "November", "December"
	];

	fullDate += monthNames[date.getMonth()] + " ";
...
//    }
	...
	TweenMax.set(moonburst.node, { opacity: 0 })
	TweenMax.set(moon.node, { opacity: 0 })
	changeWeather(weather[4]);
...
function onResize()
{
	...
	TweenMax.set(moonburst.node, {transformOrigin:"50% 50%", x: sizes.container.width / 2, y: (sizes.card.height/2) + sizes.card.offset.top});
	TweenMax.fromTo(moonburst.node, 20, {rotation: 0}, {rotation: 360, repeat: -1, ease: Power0.easeInOut})
...
function tick()
{
	...
	for(var i = 0; i < clouds.length; i++)
	{
		if(currentWeather.type == 'sun' || currentWeather.type == 'night')
		{
...
function changeWeather(weather)
{
	...
	{
		case 'night':
			TweenMax.set(moon.node, { opacity: 1 })
			TweenMax.to(moon.node, 4, {x: sizes.card.width / 2, y: sizes.card.height / 2, ease: Power2.easeInOut});
			TweenMax.to(moonburst.node, 4, {scale: 1, opacity: 0.8, y: (sizes.card.height/2) + (sizes.card.offset.top), ease: Power2.easeInOut});
			break;
		default:
			TweenMax.to(moon.node, 2, {x: sizes.card.width / 2, y: -100, leafCount: 0, ease: Power2.easeInOut});
			TweenMax.to(moonburst.node, 2, {scale: 0.4, opacity: 0, y: (sizes.container.height/2)-50, ease: Power2.easeInOut});
			break;
	}
